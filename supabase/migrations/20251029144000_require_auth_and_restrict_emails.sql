...
-- Anyone can view offers (reading is still public)
CREATE POLICY "Anyone can view active offers"
  ON public.offers
  FOR SELECT
  USING (true);  -- Show all offers, we'll filter deleted ones in the app if soft deletes are enabled
...
CREATE POLICY "Authenticated users can submit offers"
  ...
  WITH CHECK (
    ...
    auth.email() ILIKE '%.edu' OR auth.email() ILIKE '%@uwaterloo.ca'
  );
